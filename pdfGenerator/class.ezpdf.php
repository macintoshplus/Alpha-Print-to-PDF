...
function ezStartPageNumbers($x,$y,$size,$font,$r=0,$v=0,$b=0,$pos='left',$pattern='{PAGENUM} of {TOTALPAGENUM}',$num=''){
  ...
  $this->ez['pageNumbering'][$i][$this->ezPageCount]=array('x'=>$x,'y'=>$y,'pos'=>$pos,'pattern'=>$pattern,'num'=>$num,'size'=>$size, 'r'=>$r, 'v'=>$v, 'b'=>$b, 'font'=>$font);
  return $i;
}
    ...
    foreach($tmp1 as $i=>$tmp){
      ...
      foreach ($this->ezPages as $pageNum=>$id){
        ...
        if ($status){
		  ...
		  $this->saveState();
		  $this->selectFont($info['font']);
		  $this->setColor($info['r'],$info['v'],$info['b']);
          switch($info['pos']){
            case 'right':
